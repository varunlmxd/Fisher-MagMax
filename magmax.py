...
for key in task_vectors[0].keys():
    ...
    if key not in fisher_matrices[0]:
        ...
        for i in range(1, len(task_vectors)):
            ...
            # multiply task with fisher
            weighted_task_vector1 = task_vector1 * fisher_matrix1
            weighted_task_vector2 = task_vector2 * fisher_matrix2

            condition = torch.abs(weighted_task_vector2) > torch.abs(weighted_task_vector1)
            max_abs_vector = torch.where(condition, task_vector2, max_abs_vector) # select max abs vector
